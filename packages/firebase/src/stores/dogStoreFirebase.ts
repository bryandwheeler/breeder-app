...
import { Dog, NewDog, Litter } from '@breeder/types';
import { differenceInDays } from 'date-fns';
import { loadAllDogSubcollections, loadAllLitterSubcollections } from '../utils/subcollections';
...
export const useDogStore = create<Store>()((set, get) => ({
  ...
  deleteDog: async (id) => {
    ...
    // Update user profile with new dog count
    if ((dogData as any)?.userId) {
      await updateUserCounters((dogData as any).userId);
    }
  },
  ...
  deleteLitter: async (id) => {
    ...
    // Update user profile with new litter count
    if ((litterData as any)?.userId) {
      await updateUserCounters((litterData as any).userId);
    }
  },
