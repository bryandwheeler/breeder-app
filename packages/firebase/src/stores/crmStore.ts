...
interface Store {
  ...
  // Contact Search & Auto-Linking
  searchContactByEmail: (email: string) => Customer | undefined;
  searchContactByPhone: (phone: string) => Customer | undefined;
  searchContactByName: (name: string) => Customer[];
  findOrCreateContact: (data: {
    ...
    address?: string;
    roles: ContactRole[];
  }) => Promise<string>; // Returns contactId
}
...
export const useCrmStore = create<Store>()((set, get) => ({
  ...
  // Contact Search & Auto-Linking Implementation
  ...
  searchContactByPhone: (phone) => {
    ...
    return get().customers.find((c) => {
      ...
      return customerPhone.slice(-10) === normalized.slice(-10);
    });
  },
  ...
  findOrCreateContact: async (data) => {
    const user = auth.currentUser;
    if (!user) throw new Error('Must be logged in');

    // Try email match first
    let existing = data.email
      ? get().searchContactByEmail(data.email)
      : undefined;

    // Try phone match if no email match
    if (!existing && data.phone) {
      existing = get().searchContactByPhone(data.phone);
    }

    if (existing) {
      // Update roles if needed
      const currentRoles = existing.contactRoles || [];
      const newRoles = data.roles.filter((r) => !currentRoles.includes(r));

      if (newRoles.length > 0) {
        await get().updateCustomer(existing.id, {
          contactRoles: [...currentRoles, ...newRoles],
        });
      }
    ...
    > = {
      ...
      type: 'prospect', // Default type
      status: 'active',
      contactRoles: data.roles,
    };

    await get().addCustomer(customerData);

    // Find the newly created contact and return its ID
    const newContact = data.email
      ? get().searchContactByEmail(data.email)
      : get().customers.find((c) => c.name === data.name);
