...
import { Workflow, WorkflowExecutionLog, DEFAULT_WORKFLOW_TEMPLATES } from '@/types/workflow';
...
export const useWorkflowStore = create<WorkflowStore>((set, get) => ({
  ...
  executeAction: async (action, context, userId) => {
    const { useScheduledEmailStore } = await import('./scheduledEmailStore');
    ...
    switch (action.type) {
      case 'send_email':
        if (action.templateId && context.customer) {
          const { useEmailTemplateStore } = await import('./emailTemplateStore');
