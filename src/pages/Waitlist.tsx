...
function SortableWaitlistItem({ entry, onView, onDelete }: {
  ...
  const getPriorityColor = (priority?: 'standard' | 'priority' | 'vip') => {
    switch (priority) {
      case 'vip': return 'text-purple-600 font-bold';
      case 'priority': return 'text-blue-600 font-semibold';
      default: return '';
    }
  };
    ...
    <div ref={setNodeRef} style={style} className="touch-none">
      <Card className="p-4 mb-2">
        <div className="flex items-center gap-4">
          ...
          {/* Position */}
          <div className={`text-lg font-bold min-w-[40px] ${getPriorityColor(entry.priority)}`}>
            #{entry.position}
          </div>
          ...
          <div className="text-sm text-muted-foreground">
            {entry.preferredSex && (
              <div>Sex: {entry.preferredSex === 'either' ? 'Any' : entry.preferredSex}</div>
            )}
...
export function Waitlist() {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      const oldIndex = sortedWaitlist.findIndex((item) => item.id === active.id);
  ...
  const activeCount = waitlist.filter((e) => e.status === 'active' || e.status === 'approved').length;
  const depositPaidCount = waitlist.filter((e) => e.depositPaid).length;
  const matchedCount = waitlist.filter((e) => e.status === 'matched' || e.status === 'reserved').length;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="p-4">
          ...
          <div className="text-2xl font-bold text-blue-600">{depositPaidCount}</div>
        </Card>
      </div>
      ...
      {sortedWaitlist.length === 0 ? (
        <Card className="p-12 text-center">
          <p className="text-muted-foreground">No entries found. Share your waitlist application link with potential customers!</p>
        </Card>
      ) : (
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          <SortableContext items={sortedWaitlist.map((e) => e.id)} strategy={verticalListSortingStrategy}>
            <div>
